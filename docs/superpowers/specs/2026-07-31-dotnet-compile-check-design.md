...
- `app/dotnet_build_runner.py`:
  ...
  - `run_build(project_dir) -> dict`: runs `dotnet build <path> --nologo` in the discovered project's directory via `_run_subprocess`, with `DOTNET_BUILD_TIMEOUT_SECONDS` (generous enough for a cold-cache NuGet restore + build, same scale as Android's `GRADLE_TIMEOUT_SECONDS`). Returns `{"returncode": int|None, "stdout": str, "stderr": str}` — success/failure is decided by the caller from parsed diagnostics, not the exit code, same as every other runner this session.
